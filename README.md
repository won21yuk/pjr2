...
- 이에 따라 본 프로젝트는 대여소별 시공간적 특성을 활용한 이용량 예측 및 자전가 재배치를 위한 데이터 파이프라인 구축을 목표로 진행하였음
...
3. 파이프 라인 운영
   ...
   - DAG들의 DEPENDECY 설정
     - Raw data 수집하여 aws 홈디렉토리에 저장(python operator) -> HDFS로 적재(bash operator) -> 가공 및 DB적재(spark-submit operator) 
