...
- DB 이원화
  ...
  - MongoDB
    - 위치정보가 포함된 데이터
    - 모든 위치 데이터는 위치 필터링에 용이하도록 GEO JSON 형태로 가공하여 MongoDB에 적재
      - 중심값과 반경값만 입력하면 해당 컬렉션의 도큐먼트 중 반경 내에 위치한 값들을 조회 가능한 위치기반 쿼리(geometry) 사용목적 
